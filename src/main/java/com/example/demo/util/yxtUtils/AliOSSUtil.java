...
public class AliOSSUtil {
    ...
    static {
        //初始化AccessKey
        accessKeyId = OSSProperties.ALIYUN_ACCESS_ID;
        //初始化AccessKeySecret
        accessKeySecret = OSSProperties.ALIYUN_ACCESS_KEY;
        //初始化Endpoint
        endpoint = OSSProperties.ALIYUN_ENDPOINT;
        //初始化bucketName
...
/**
    ...
    /**
     * 2
     * 以流的方式读取一个文件 并获取文件名、最后修改时间
     *
     * @param fileURL 文件的url*/


    public static List<String> downloadStream2(String fileURL) {
        //<yourObjectName>表示从OSS下载文件时需要指定包含文件后缀在内的完整路径，例如abc/efg/123.jpg。
        String objectName = getObjectName(fileURL);
        ...
        //获取key
        String substring = ossObject.getKey().substring(32);
        //获取该文件最后的修改时间信息
        Date lastModified = ossObject.getObjectMetadata().getLastModified();
        System.out.println(lastModified);
        String ft2 = commonUtils.getFormatTime(lastModified);
